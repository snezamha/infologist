...
import { PageEditorView } from "@/features/page-builder/components/page-editor-view";

type Props = {
  params: Promise<{ domainId: string; pageId: string }>;
};

export default async function EditProjectPageBuilderPage({ params }: Props) {
  ...
  if (!user) {
    redirect(
      buildProjectAuthHref(
        ...
        `/dashboard/page-builder/${pageId}/edit`,
      ),
    );
  }
